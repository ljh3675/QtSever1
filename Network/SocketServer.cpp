...
SocketServer::SocketServer()
{
    ...
    for(int i = 0; i < 20; i++)//尝试不同的端口号
    {
        ...
        {
            MY_LOG("mytcpserver listen failed!!");
        }
    }

    connect(&m_diagnoseTimer,SIGNAL(timeout()),this,SLOT(socketTimeOut()));

    MY_DEBUG("SocketServer() finished");
}
...
void SocketServer::OnNewConnection()
{
    ...
    {
        MY_DEBUG("sendTestInfoSocket OnNewConnection");
        MY_DEBUG("socketDescriptor="<<newSocket->socketDescriptor());
